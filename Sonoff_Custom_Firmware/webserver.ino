...
void handleApiRoot() {
	server.send(200, "text/html", " \
								  <html> \
								  <head> \
								  <title>SmartHome Switch Api</title> \
								  ...
								  <div id='main'> \
								  <h1>API</h1> \
								  <h2>SmartHome Switch Controller</h2> \
...
void handleApiGet() {
	...
	server.send(200, "application/json", "{\"power\":" + String(relaisOn?"true":"false") + ", \"mac\":\"" + getMacString() + "\"}");
}
...
void handleApiSet() {
	...
	if (key != "") {
		...
		if (encryptDecrypt(hex_to_string(key)) == apiKey) {
			...
			if (power != "") {
				if (!isNumeric(power) || power.toInt() < 0 || power.toInt() > 1)
				{
					error = "power outside of valid range";
				}
				else
				{
					if (power == "1")
